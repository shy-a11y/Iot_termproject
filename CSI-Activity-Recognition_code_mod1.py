...
def extract_csi_by_label(raw_folder, label, labels, save=False, win_len=1000, thrshd=0.6, step=200):
    ...
    if not label in labels:
        raise ValueError("The label {} should be among 'empty','stand','sit', 'walk','fall'".format(labels))
...
def train_valid_split(numpy_tuple, train_portion=0.9, seed=379):
    ...
    for i, x_arr in enumerate(numpy_tuple):
        ...
        tmpy = np.zeros((split_len,5))
        tmpy[:, i] = 1
        y_train.append(tmpy)
        x_valid.append(x_arr[index[split_len:],...])
        tmpy = np.zeros((x_arr.shape[0]-split_len,5))
        tmpy[:, i] = 1
        y_valid.append(tmpy)
...
class CSIModelConfig:
    ...
    def __init__(self, win_len=1000, step=200, thrshd=0.6, downsample=2):
        ...
        self._step = step
        self._thrshd = thrshd
        self._labels = ("empty","stand","sit","walk","fall")
    ...
    def load_csi_data_from_files(self, np_files):
        ...
        Returns the Numpy Array for training within the format of (X_lable1, y_label1, ...., X_label7, y_label7)
        Args:
            np_files: ('x_empty.npz','x_bed.npz', 'x_fall.npz', 'x_pickup.npz', 'x_run.npz', 'x_sitdown.npz', 'x_standup.npz', 'x_walk.npz')
        """
        if len(np_files) != 5:
            raise ValueError('There should be 5 numpy files for empty,stand,sit, walk,fall')
...
if __name__ == "__main__":
    ...
    numpy_tuple = cfg.preprocessing('motion_result_1.csv', save=True)
    # load previous saved numpy files, ignore this if you haven't saved numpy array to files before
    # numpy_tuple = cfg.load_csi_data_from_files(('x_bed.npz', 'x_falsl.npz', 'x_pickup.npz', 'x_run.npz', 'x_sitdown.npz', 'x_standup.npz', 'x_walk.npz'))
    x_empty, y_empty, x_stand , y_stand, x_sit, y_sit, x_walk, y_walk, x_fall, y_fall, = numpy_tuple
    x_train, y_train, x_valid, y_valid = train_valid_split(
        (x_empty,x_stand, x_sit, x_walk, x_fall),
